...
import { getCompOrderEnquiries } from "@/lib/api-routes/api-admin";
...
const ResolvedOrderEnquiries = () => {
  ...
  const fetchResolvedEnquiries = useCallback(async () => {
    try {
      dispatch(showLoader());
      const response = await getCompOrderEnquiries(paginationData);
      setEnquiriesData({
          ...
          {enquiriesData.data?.length ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">
                There are no completed order enquiries at the moment.
              </p>
            </div>
          )}
        </div>
...
export default ResolvedOrderEnquiries;
